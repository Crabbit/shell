...
function deploy(){
    ...
    for i in rts rts_noanti rtswb rtstb wiserts; do
        nohup sh -x ${BIN_DIR}/sofa_deploy_job -k ${i} -t cs -d "${time_arg}" -r </dev/null &>"${LOG_DIR}/register_app_${i}_cs.log"
        ret="$?"
        wait
        [ "$ret" -ne "0" ] && FATAL_WARN "deploy $i parser ${time_arg} failed"
        #nohup sh -x ${BIN_DIR}/sofa_deploy_job -k ${i} -t cs -d "${time_arg}"</dev/null $>"${LOG_DIR}/deploy_job_${i}_cs.log"
        #wait
    done

    nohup sh -x ${BIN_DIR}/sofa_deploy_job -t es -d "${time_arg}" -r </dev/null &>"${LOG_DIR}/register_app_es.log"
    ret="$?"
    [ "$ret" -ne "0" ] && FATAL_WARN "deploy dag ${time_arg} failed"
    wait
    nohup sh -x ${BIN_DIR}/sofa_deploy_job -t es1 -d "${time_arg}" </dev/null &>"${LOG_DIR}/deploy_es1_job_${time_arg}.log"
    ret="$?"
    wait
    [ "$ret" -ne "0" ] && FATAL_WARN "deploy antipsam ${time_arg} failed"
    nohup sh -x ${BIN_DIR}/sofa_deploy_job -t es2 -d "${time_arg}" </dev/null &>"${LOG_DIR}/deploy_es2_job_${time_arg}.log"
    ret="$?"
    wait
    [ "$ret" -ne "0" ] && FATAL_WARN "deploy extractor ${time_arg} failed"
...
function start_app(){
    ...
    for i in rts rts_noanti rtswb rtstb wiserts; do
        ...
        wait
        if [ "$ret" -eq 0 ]; then
            touch "${FLAG_DIR}/${i}_${time_arg}.start"
        else
            FATAL_WARN "start $i app failed."
        fi
    done
    ...
    ret="$?"
    wait
    [ "$ret" -ne "0" ] && FATAL_WARN "start dag pp failed."
