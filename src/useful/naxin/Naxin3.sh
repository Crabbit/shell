...
cat $CHANGE_ROOT/etc/shadow |grep lili >> /dev/null
if [ "$?" == "0" ];then
                Que4_Result=0
else
                Que4_Result=1
...
cat $CHANGE_ROOT/etc/group |grep lili |grep root |grep last >> /dev/null
if [ "$?" == "0" ];then
                Que4_Result=0
...
#########################################
# 6th question
if [ "$( cat $CHANGE_ROOT/etc/fstab | grep "rhel-server-6.3-x86_64.iso" | cut -d ' ' -f 1 )" == "$CHANGE_ROOT/home/last/rhel-server-6.3-x86_64.iso" ] && [ "$( cat $CHANGE_ROOT/etc/fstab | grep "rhel-server-6.3-x86_64.iso" | cut -d ' ' -f 3-4 )" == "iso9660 loop" ];then
